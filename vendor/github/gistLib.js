...
// To save keystrokes when we make JSONP calls to the HTTP API, we will keep track of the root from which all V2 urls extend.
    var authUsername, authToken, apiRoot = "https://github.com/api/v2/json/",

// Send a JSONP request to the Github API that calls callback with the context argument as this.
// The url parameter is concatenated with the apiRoot, for the reasons mentioned above. 
// The way that we are supporting non-global, anonymous functions is by sticking them in the 
// globally exposed gh.__jsonp_callbacks object with a "unique" id that is the current time 
// in milliseconds. Once the callback is called, it is deleted from the object to prevent memory leaks.
...
// This is implemented with a hack to get around the cross-domain restrictions on ajax calls. Basically, a form is created that 
// will POST to the GitHub API URL, stuck inside an iframe so that it won't redirect this page, and then submitted.

    post = function (url, vals) {
        var
        form = document.createElement("form"),
        iframe = document.createElement("iframe"),
        doc = iframe.contentDocument !== undefined ? iframe.contentDocument : iframe.contentWindow.document,
        key, field;
        vals = vals || {};
...
// Convert an object to a url parameter string.
    ...
    paramify({foo:1, bar:3}) -> "foo=1&bar=3".paramify = function (params) {
        var str = "", key;
...
// The constructor for user objects. Just creating an instance of a user doesn't fetch any data from GitHub, you need to get explicit
// about what you want to do that.
...
// Get a list of all repos that this user can push to (including ones that they are just a collaborator on, and do not own). 
...
// This is the base constructor for creating repo objects. Note that this won't actually hit the GitHub API until you specify 
// what data you want, or what action you wish to take via a prototype method.
